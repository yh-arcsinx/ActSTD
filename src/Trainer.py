...
class Trainer(object):
    def __init__(self, args,device,TIME):
        ...
        if True:
        # if args.run_name!='debug':#args里面没有，有debug 和！debug
    ...
    def model_init(self):
        ...
        self.model = SelfAttentiveCNFSpatiotemporalModel(dim=x_dim,
            hidden_dims=list(map(int, self.args.hdims.split("-"))), #[64,64,64]
            tpp_hidden_dims=list(map(int, self.args.tpp_hdims.split("-"))), #[32,32]
