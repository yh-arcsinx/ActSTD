...
def args_set():
    ...
    #后面使用前面缺失的 '--run_name'default='debug'
...
def initial_setting():
    ...
    os.environ['PYTHONHASHSEED'] = str(args.seed) #GPU迭代相同可复现
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    torch.cuda.manual_seed(args.seed)
    torch.backends.cudnn.benchmark = False  #GPU加速库 适合于网络形状不变的
...
if __name__ == '__main__':
    ...
    Trainer = Trainer(args, device, TIME)
    Trainer.Train()
